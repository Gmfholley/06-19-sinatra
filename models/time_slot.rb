class TimeSlot < ActiveRecord::Base
  ...
  def num_staff_needed
    sum = 0
    query_string = "SELECT SUM(locations.num_staff) FROM location_times INNER JOIN time_slots ON location_times.time_slot_id = time_slots.id INNER JOIN locations ON location_times.location_id = locations.id WHERE time_slots.id = #{self.id} GROUP BY location_times.location_id;"
